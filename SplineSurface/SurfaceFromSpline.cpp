...
SurfaceFromSpline::SurfaceFromSpline(std::string vertexShader, std::string fragmentShader) :Surface(vertexShader, fragmentShader)
{
	p = Point(0.f,0.f,-4.f);
	step = 20;
	ratio = 0.1f;
...
SurfaceFromSpline::~SurfaceFromSpline()
{
...
void SurfaceFromSpline::SimpleExtrude()
{
...
void SurfaceFromSpline::ExtrudeFromAxis()
{
	if (m_listCourbes.empty() || m_listCourbes[0].size() < 2)
		return;
...
void SurfaceFromSpline::ExtrudeFromSoul(std::shared_ptr<Courbe> soul)
{
	/*if (m_listCourbes.empty() || m_listCourbes[0].size() < 2)
		return;
...
void SurfaceFromSpline::computePointInCourbe()
{
	//SimpleExtrude();
	ExtrudeFromAxis();
}

void SurfaceFromSpline::load()
{
...
void SurfaceFromSpline::drawCourbe(Mat4x4 projection, Mat4x4 modelView)
{
